...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        int numOfCustomer = Integer.parseInt(br.readLine());
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(System.out));
        for (int i = 0; i < numOfCustomer; i++) {
            String [] arr = br.readLine().split(" ");
            Input input = new Input(Integer.parseInt(arr[0]),Integer.parseInt(arr[1]), Integer.parseInt(arr[2]));

            int roomHeight =  input.num % input.height;
            int roomNum = input.num / input.height + 1;

            if (roomHeight == 0) {
                roomHeight = input.height;
                roomNum = input.num / input.height;
            }

            String roomHeightstr = Integer.toString(roomHeight);
            String roomNumstr;
            if (roomNum < 10) {
                roomNumstr = 0 + Integer.toString(roomNum);
            } else {
                roomNumstr = Integer.toString(roomNum);;
            }
            String result = roomHeightstr + roomNumstr;
            bw.write(result);
            bw.newLine();
        }
        bw.flush();
...
class Input {
    ...
    public Input(int height, int width, int num){
        this.height = height;
        this.width = width;
        this.num = num;
    }
